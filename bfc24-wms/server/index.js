...
async function loadShipmentLinesForPackingOrShipping(db, shipmentExternalId) {
  const { rows } = await db.query(
    `
    SELECT
      pt.barcode,
      COALESCE(wbi.item_name, wbi.title, pt.barcode) AS item_name,
      SUM(pt.qty) AS qty,
    ...
    LEFT JOIN public.mp_wb_orders o
      ON  o.barcode     = pt.barcode
      AND o.supply_code = pt.shipment_code
    WHERE pt.shipment_code = $1
      AND pt.status = 'done'
    GROUP BY
      pt.barcode,
      COALESCE(wbi.item_name, wbi.title, pt.barcode),
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const [, group] of groups.entries()) {
        ...
        const supplyId = supplyBody.id || supplyBody.supplyId || supplyBody.supply_id;
        if (!supplyId) throw new Error('WB create supply: supplyId is missing in response');

        // 4.2 добавляем заказы в поставку
        const addRes = await wbAddOrdersToSupply({ apiToken, supplyId, orderIds });
        ...
        try {
          ...
          // 5.3 создаём/обновляем WMS задачи на сборку по поставке
          let tasksInserted = 0;
          let tasksUpdated = 0;

          // агрегируем количество по wb_order_id + barcode
          ...
          for (const row of aggRes.rows) {
            ...
            // защита от повторной генерации: либо обновляем qty, либо вставляем новую задачу
            const existing = await db.query(
              `
                ...
                LIMIT 1
              `,
              [wmsClientId, wbOrderId, barcodeStr, String(supplyId)],
            );
          ...
          // 5.4 создаем/обновляем волну на сборку
          await db.query(
            `
            ...
            `,
            [wmsClientId, String(supplyId)],
          );

          // 5.5 создаём задание на упаковку
          await db.query(
            `
...
// - создаём задание на упаковку wms.shipments_wb (status = 'ready_for_packing')
// -------------------------
app.post(
  '/picking/generate-from-wb',
  authRequired,
  ...
  async (req, res) => {
    ...
    async function wbTryFetchStickers({ apiToken, orderIds, type = 'svg', width = 58, height = 40 }) {
      ...
      for (const authHeader of authVariants) {
        ...
        if (!resp.ok) {
          ...
          if (resp.status === 401) continue; // пробуем другой формат авторизации
          return last;
        }
    ...
    try {
      ...
      for (const [, group] of groups.entries()) {
        ...
        await db.query('BEGIN');
        try {
          // 5.1 отмечаем поставку в mp_wb_orders
          await db.query(
            `
              UPDATE public.mp_wb_orders
                 SET wb_supply_id = $1,
          ...
          for (const o of group.orders) {
            ...
            // ⚠️ защита от дублей ПО ВОЛНЕ:
            // не даём создать вторую задачу на тот же заказ внутри этого shipment_code
...
// -------------------------
// Получить текущее задание на упаковку
// -------------------------
app.get(
  '/packing/current',
  authRequired,
  ...
  async (req, res) => {
    try {
      ...
      if (!rows.length) {
        return res.status(404).json({ message: 'Нет активного задания' });
      }
      ...
      const linesResult = await pool.query(
        ...
        LEFT JOIN public.mp_wb_items wbi
          ON wbi.nm_id = ib.nm_id
        LEFT JOIN public.mp_wb_orders o
          ON o.barcode = pt.barcode               -- ТОЛЬКО ПО BARCODE
        WHERE pt.shipment_code = $1
          AND pt.status = 'done'
        GROUP BY
          pt.barcode,
...
// -------------------------
// Взять следующее задание на упаковку
// -------------------------
app.post(
  '/packing/next',
  authRequired,
  ...
  async (req, res) => {
    try {
      ...
      if (!nextResult.rows.length) {
        ...
        return res
          .status(404)
          .json({ message: 'Нет доступных заданий на упаковку' });
      }

      const shipment = nextResult.rows[0];
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      const { shipment_id, location_code } = req.body || {};
      const sid = Number(shipment_id);

      if (!sid) {
        client.release();
        return res.status(400).json({ message: 'shipment_id обязателен' });
      }
      if (!location_code) {
        client.release();
        return res.status(400).json({ message: 'location_code (МХ стола) обязателен' });
      }

      await client.query('BEGIN');

      // 1. Проверяем МХ (должна существовать и быть активной).
      const locRes = await client.query(
        `
        SELECT code, zone, is_active
        FROM wms.locations
        WHERE code = $1
        `,
        [location_code]
      ...
      if (!locRes.rows.length) {
        await client.query('ROLLBACK');
        client.release();
        return res.status(400).json({ message: 'МХ не найдена' });
      }
      ...
      if (!loc.is_active) {
        await client.query('ROLLBACK');
        client.release();
        return res.status(400).json({ message: 'МХ не активна' });
      }

      // Здесь мы уже на фронте фильтруем по зоне "Упаковка",
      // но можно при желании дожать и тут:
      // if (loc.zone !== 'Упаковка') { ... }
      ...
      if (shipment.status !== 'packing') {
        ...
        return res
          .status(400)
          .json({ message: `Ожидался статус "packing", сейчас "${shipment.status}"` });
      }

      // 3. Переводим в packed и запоминаем МХ стола.
      const updRes = await client.query(
        `
        UPDATE wms.shipments
        SET status = 'packed',
            packing_location_code = $2,
            updated_at = NOW()
        WHERE id = $1
        RETURNING *
        `,
      ...
      const updatedShipment = updRes.rows[0];

      // 4. (опционально) пишем движение "packing" — единым блоком по отгрузке.
      // Это движение НЕ меняет остатки, только даёт историю.
      const totalQtyRes = await client.query(
        ...
        WHERE shipment_code = $1
        `,
        [updatedShipment.external_id]
      );

      const totalQty =
        (totalQtyRes.rows.length && Number(totalQtyRes.rows[0].total_qty)) || 0;
      ...
      await client.query(
        ...
        VALUES
          (NOW(), $1, $2, NULL, $3,
           NULL, $4,
           'packing', 'shipment', $5, 'Упаковка заказа на стол', NULL)
        `,
        [req.user.id, updatedShipment.client_id, totalQty, location_code, updatedShipment.id]
      );
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      if (!rows.length) {
        return res.status(404).json({ message: 'Нет активного задания на отгрузку' });
      }
...
app.post(
  ...
  async (req, res) => {
    const client = await pool.connect();
    try {
      // Сначала смотрим, нет ли уже "shipping" в работе
      ...
      // Берём следующую упакованную отгрузку
      ...
      if (!nextRes.rows.length) {
        ...
        return res
          .status(404)
          .json({ message: 'Нет доступных заданий на отгрузку' });
      }
...
// -------------------------
// Подтверждение отгрузки (скан WG-кода)
// -------------------------
app.post(
  '/shipping/confirm',
  authRequired,
  ...
  async (req, res) => {
    ...
    if (!shipment_id) {
      return res.status(400).json({ message: 'Не передан shipment_id' });
    }
    if (!wg_code || typeof wg_code !== 'string') {
      return res.status(400).json({ message: 'Не передан WG-код' });
    }
    ...
    try {
      ...
      // 1. Берём отгрузку под блокировку
      const { rows, rowCount } = await client.query(
        `SELECT id, client_id, external_id, status
         FROM wms.shipments
         WHERE id = $1
         FOR UPDATE`,
        [shipment_id]
      ...
      // 2. Проверяем статус и WG-код
      const expectedStatus = 'shipping';
      if (shipment.status !== expectedStatus) {
        await client.query('ROLLBACK');
        return res.status(400).json({
          message: `Отгрузка в статусе "${shipment.status}", ожидали "${expectedStatus}"`,
        });
      }

      const trimmed = wg_code.trim();
      if (shipment.external_id !== trimmed) {
        await client.query('ROLLBACK');
        return res.status(400).json({
          message: 'WG-код не совпадает с кодом отгрузки',
        });
      }

      // 3. Движение "shipping" в wms.movements
      await client.query(
        `INSERT INTO wms.movements
          (created_at, user_id, client_id, barcode, qty,
           from_location, to_location,
         ...
         VALUES
          (now(), $1, $2, NULL, 1,
           NULL, NULL,
           'shipping', 'shipment', $3, 'Отгрузка перевозчику', NULL)`,
        [userId, shipment.client_id, shipment.id]
      );

      // 4. Обновляем статус отгрузки + кто/когда отгрузил
      ...
      // 5. Вызов WB API: передать поставку в доставку и получить QR
      let qrBase64 = null;
      try {
        ...
        const { qrBase64: qrFromWb } = await wbService.confirmShipmentAndGetQr({
          ...
          shipmentCode: shipment.external_id, // WB-GI-...
        });

        qrBase64 = qrFromWb || null;

        console.log(
          '[shipping/confirm] WB QR base64 length =',
          qrBase64 ? qrBase64.length : 0,
          'prefix =',
          qrBase64 ? qrBase64.slice(0, 40) : null
        );

        // 5.1. Сохраняем QR в отгрузке для перепечатки
        if (qrBase64) {
          await client.query(
            `UPDATE wms.shipments
        ...
        // Отгрузку всё равно считаем подтверждённой.
      }

      await client.query('COMMIT');
      return res.json({ ok: true, qr_base64: qrBase64 });
    } catch (err) {
      await client.query('ROLLBACK');
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!pickerId) return res.status(401).json({ error: 'Не определён пользователь' });
      if (!taskId || !scanned) {
        return res.status(400).json({ error: 'picking_task_id и scanned_barcode обязательны' });
      }
      ...
      const taskRes = await db.query(
        ...
        SELECT
          id, client_id, barcode, sku_id, location_code, qty,
          status, picker_id, scan_step, picked_qty,
          comment AS task_comment
        FROM wms.picking_tasks
        WHERE id=$1
        FOR UPDATE
      ...
      if (step !== 'await_item') {
        await db.query('ROLLBACK');
        return res.status(409).json({ error:'Ожидается скан товара', scan_step: step });
      }

      const expectedBarcode = String(t.barcode || '').trim();
      if (!expectedBarcode) {
        await db.query('ROLLBACK');
        return res.status(400).json({ error:'В задаче пустой barcode' });
      }
      ...
      // если ещё не добрали — просто обновим прогресс
      if (pickedQty < qtyToPick) {
        await db.query(
          `
          UPDATE wms.picking_tasks
          SET picked_qty=$1, updated_at=NOW(), updated_by=$2
          WHERE id=$3
          `,
        ...
        await db.query('COMMIT');
        return res.json({ result:'ok', picked_qty:pickedQty, qty:qtyToPick, next_step:'await_item' });
      }
      ...
      if (t.sku_id !== null && t.sku_id !== undefined) {
        ...
        skuIdFinal = await resolveSkuIdOrCreate(db, { client_id: clientIdNum, barcode: barcodeStr });
      }
      ...
      if (locCode) {
        ...
        if (!locIdRes.rowCount) throw new Error(`Не найдена ячейка: ${locCode}`);
        ...
        if (!locRes.rowCount) throw new Error('Не найдена ячейка с остатком для SKU (по клиенту)');
      ...
      const stockRes = await db.query(
        `SELECT qty FROM wms.stock WHERE location_id=$1 AND sku_id=$2 FOR UPDATE`,
        [locationId, skuIdFinal]
      );
      if (!stockRes.rowCount) throw new Error('В ячейке нет остатка по этому SKU');

      const currentQty = Number(stockRes.rows[0].qty || 0);
      if (currentQty < qtyToPick) throw new Error(`Недостаточно остатка: есть ${currentQty}, нужно ${qtyToPick}`);

      const newQty = currentQty - qtyToPick;
      await db.query(
        `UPDATE wms.stock SET qty=$1 WHERE location_id=$2 AND sku_id=$3`,
      ...
      const upd = await db.query(
        ...
        SET
          ...
          updated_by=$4
        WHERE id=$5
        RETURNING id, status, picked_qty, qty, location_code, sku_id
        `,
      ...
      return res.status(500).json({ error:'scan/item error', detail:e.message });
    } finally {
      db.release();
    }
  }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!Number.isInteger(qtyToPick) || qtyToPick <= 0) {
        ...
        return res
          .status(400)
          .json({ error: 'Некорректное qty в задаче на сборку' });
      }
      ...
      // 8) Автоматически создаём задачу на упаковку (отдельным коннектом)
console.log(
  '[picking/confirm] call autoCreatePackingTaskForShipment',
  {
